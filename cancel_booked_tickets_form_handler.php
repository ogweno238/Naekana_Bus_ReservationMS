		...
		<?php
			if(isset($_POST['Cancel_Ticket']))
			{
				...
				if(empty($data_missing))
				{
					...
					if($affected_rows==1)
					{
						$query="SELECT t.bus_no,t.journey_date,t.no_of_passengers,t.class,0.85*p.payment_amount as refund_amount from ticket_details t,payment_details p WHERE t.pnr=? and t.pnr=p.pnr";
						$stmt=mysqli_prepare($dbc,$query);
						mysqli_stmt_bind_param($stmt,"s",$pnr);
						mysqli_stmt_execute($stmt);
						mysqli_stmt_bind_result($stmt,$bus_no,$journey_date,$no_of_pass,$class,$refund_amount);
						mysqli_stmt_fetch($stmt);
						mysqli_stmt_close($stmt);
						$_SESSION['refund_amount']=$refund_amount;
						if($class=='economy')
						{
							$query="UPDATE bus_details SET seats_economy=seats_economy+? WHERE bus_no=? AND departure_date=?";
							$stmt=mysqli_prepare($dbc,$query);
							mysqli_stmt_bind_param($stmt,"iss",$no_of_pass,$bus_no,$journey_date);
							mysqli_stmt_execute($stmt);
						...
						{
							$query="UPDATE bus_details SET seats_business=seats_business+? WHERE bus_no=? AND departure_date=?";
							$stmt=mysqli_prepare($dbc,$query);
							mysqli_stmt_bind_param($stmt,"iss",$no_of_pass,$bus_no,$journey_date);
							mysqli_stmt_execute($stmt);
							$affected_rows_1=mysqli_stmt_affected_rows($stmt);
