...
<html>
	<head>
		<title>Add Bus Schedule Details</title>
	</head>
	<body>
		<?php
			if(isset($_POST['Submit']))
			{
				...
				if(empty($data_missing))
				{
					...
					if($cnt==1)
					{
						$query="INSERT INTO bus_Details (bus_no,from_city,to_city,departure_date,arrival_date,departure_time,arrival_time,seats_economy,seats_business,price_economy,price_business,tablet_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
						$stmt=mysqli_prepare($dbc,$query);
						mysqli_stmt_bind_param($stmt,"sssssssiiiis",$bus_no,$origin,$destination,$dep_date,$arr_date,$dep_time,$arr_time,$seats_eco,$seats_bus,$price_eco,$price_bus,$tablet_id);
						mysqli_stmt_execute($stmt);
					...
					if($affected_rows==1)
					{
						...
						header("location: add_bus_shedule.php?msg=success");
					}
					else
					{
						echo "Submit Error";
						echo mysqli_error();
						header("location: add_bus_shedule.php?msg=failed");
					}
				}
				else
				{
